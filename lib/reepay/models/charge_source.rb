...
module Reepay
  ...
  class ChargeSource
    # Type of charge source: `card`, `card_token`, `mobilepay` or `viabill`
    attr_accessor :type
    ...
    # Card acquirer or card payment gateway used if card source: `reepay`, `clearhaus`, `nets`, `dibs`, `stripe`, `quickpay`, `epay`, `test`
    attr_accessor :provider

    # Reference to authorization transaction if charge is settled after authorization
    attr_accessor :auth_transaction

    # Card type if credit card source: `unknown`, `visa`, `mc`, `dankort`, `visa_dk`, `visa_elec`, `maestro`, `laser`, `amex`, `diners`, `discover` or `jcb`
    ...
    # Status for strong customer authentication: `threed_secure`, `threed_secure_not_enrolled, `threed_secure_local_only`, `secured_by_nets`
    attr_accessor :strong_authentication_status

    # Card acquirer error code in case of card error
    attr_accessor :acquirer_code

    # Card acquirer message in case of card error
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'auth_transaction')
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["card", "card_token", "mobilepay", "viabill"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["card", "card_token", "mobilepay", "viabill"])
    ...
    def hash
      [type, card, fingerprint, provider, auth_transaction, card_type, exp_date, masked_card, strong_authentication_status, acquirer_code, acquirer_message, acquirer_reference, text_on_statement].hash
    end
